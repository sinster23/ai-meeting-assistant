...
router.get("/", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const recordings = await MeetingModel.find({ userId: "anonymous", source: "recording" })
      .sort({ createdAt: -1 })
      .limit(100)
      .lean();
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const meetings = await MeetingModel.find({ userId: "anonymous", source: "recording" }).lean();

      const totalRecordings = meetings.length;
      const totalDurationSeconds = meetings.reduce(
        (sum, m) => sum + ((m as any).durationSeconds ?? 0),
        0
      );
      const totalSizeBytes = meetings.reduce(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const meeting = await MeetingModel.findOneAndUpdate(
        { _id: req.params.id, userId: "anonymous" },
